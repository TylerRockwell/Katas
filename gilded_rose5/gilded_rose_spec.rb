...
describe "#update_quality" do
  ...
  context "with a single item" do
    ...
    let(:item) { NormalItem.new(name, initial_sell_in, initial_quality) }
    ...
    context "Aged Brie" do
      let(:name) { "Aged Brie" }
      let(:item) { AgedItem.new(name, initial_sell_in, initial_quality) }
    ...
    context "Sulfuras" do
      let(:legendary) { LegendaryItem.new(name, initial_sell_in, initial_quality)}
      let(:initial_quality) { 80 }
      let(:name) { "Sulfuras, Hand of Ragnaros" }

      describe "#sell_in" do
        it "doesn't change" do
          expect(legendary.sell_in).to eq(initial_sell_in)
        end
      end
      ...
      describe "#quality" do
        context "before sell date" do
          it "doesn't change" do
            expect(legendary.quality).to eq(initial_quality)
          end
        end
    ...
    context "Backstage pass" do
      ...
      let(:item) { TimeSensitiveItem.new(name, initial_sell_in, initial_quality)}
    ...
    context "conjured item" do
      ...
      let(:item) { ConjuredItem.new(name, initial_sell_in, initial_quality) }
